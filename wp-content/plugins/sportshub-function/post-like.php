...
function sportshub_like_scripts() {
	if( !defined('sportshub_PATH') ) { define('sportshub_PATH', ( plugins_url() . '/sportshub-functions') ); } 
	wp_enqueue_script( 'sportshub_like_post',  plugin_dir_url( __FILE__ ) . 'js/post-like.js', array('jquery'), '1.0', 1 );
	wp_localize_script( 'sportshub_like_post', 'ajax_var', array(
		'url' => admin_url( 'admin-ajax.php' ),
		'nonce' => wp_create_nonce( 'ajax-nonce' )
		)
	);
}
add_action( 'init', 'sportshub_like_scripts' );

/**
 * (3) Save like data
 */
add_action( 'wp_ajax_nopriv_jm-post-like', 'sportshub_post_like' );
add_action( 'wp_ajax_jm-post-like', 'sportshub_post_like' );
function sportshub_post_like() {
	$nonce = $_POST['nonce'];
    if ( ! wp_verify_nonce( $nonce, 'ajax-nonce' ) )
        die ( 'Nope!' );
	
	if ( isset( $_POST['sportshub_post_like'] ) ) {
		...
		if ( is_user_logged_in() ) { // user is logged in
			...
			if ( !sportshub_AlreadyLiked( $post_id ) ) { // like the post
				...
				print $post_like_count; // update count on front end
 ...
 */
function sportshub_AlreadyLiked( $post_id ) { // test if user liked before
	if ( is_user_logged_in() ) { // user is logged in
		$user_id = get_current_user_id(); // current user
		$meta_USERS = get_post_meta( $post_id, "_user_liked" ); // user ids from post meta
 ...
 * (5) Front end button
 */
function sportshub_getPostLikeLink( $post_id ) {
	$like_count = get_post_meta( $post_id, "_post_like_count", true ); // get post likes
	$count = ( empty( $like_count ) || $like_count == "0" ) ? '0' : esc_attr( $like_count );
	if ( sportshub_AlreadyLiked( $post_id ) ) {
 ...
 */
add_action( 'show_user_profile', 'sportshub_show_user_likes' );
add_action( 'edit_user_profile', 'sportshub_show_user_likes' );
function sportshub_show_user_likes( $user ) { ?>        
    <table class="form-table">
        <tr>
			<th><label for="user_likes"><?php esc_html__( 'You Like:', 'sportshub' ); ?></label></th>
			<td>
            <?php
			...
			if ( $count > 0 ) {
				...
				echo "<p>" . esc_html__( 'You don\'t like anything yet.', 'sportshub' ) . "</p>\n";
			} ?>
            </td>
		</tr>
    </table>
 ...
 */
function sportshub_frontEndUserLikes() {
	if ( is_user_logged_in() ) { // user is logged in
		$like_list = '';
		$user_id = get_current_user_id(); // current user
		...
		if ( $count > 0 ) {
			...
			$like_list .= "<aside>\n";
			$like_list .= "<h3>" . esc_html__( 'You Like:', 'sportshub' ) . "</h3>\n";
			$like_list .= "<ul>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function sportshub_most_popular_today() {
	global $post;
	$today = date('j');
  	$year = date('Y');
	$args = array(
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html__( 'Today\'s Most Popular Posts', 'sportshub' ) . "</h3>\n";
		echo "<ul>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function sportshub_most_popular_month() {
	global $post;
	$month = date('m');
  	$year = date('Y');
	$args = array(
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html__( 'This Month\'s Most Popular Posts', 'sportshub' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . esc_url(get_permalink($post->ID)) . "'>" . get_the_title() . "</a></li>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function sportshub_most_popular_week() {
	global $post;
	$week = date('W');
  	$year = date('Y');
	$args = array(
	...
	if ( $pop_posts->have_posts() ) {
		echo "<aside>\n";
		echo "<h3>" . esc_html__( 'This Week\'s Most Popular Posts', 'sportshub' ) . "</h3>\n";
		echo "<ul>\n";
		while ( $pop_posts->have_posts() ) {
			$pop_posts->the_post();
			echo "<li><a href='" . esc_url(get_permalink($post->ID)) . "'>" . get_the_title() . "</a></li>\n";
 ...
 * Markup assumes sidebar/widget usage
 */
function sportshub_most_popular() {
	global $post;
	echo "<aside>\n";
	echo "<h3>" . esc_html__( 'Most Popular Posts', 'sportshub' ) . "</h3>\n";
	echo "<ul>\n";
	$args = array(
		 'post_type' => array( 'post', 'enter-your-comma-separated-post-types-here' ),
		 'meta_key' => '_post_like_count',
