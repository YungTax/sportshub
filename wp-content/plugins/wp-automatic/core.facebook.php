...
class WpAutomaticFacebook extends wp_automatic
{
    ...
    public function fb_get_post($camp)
    {
        ...
        if ((wp_automatic_trim($cg_fb_page_id) != '')) {
            ...
            //retport page Id
            echo '<br>FB ID of the page:' . $cg_fb_page_id;
            ...
            if ($cg_fb_source == 'profile') {
                ...
                $cg_fb_page_feed2 = 'https://web.facebook.com/groups/' . $cg_fb_page_id . '/?sorting_setting=CHRONOLOGICAL';
            ...
            if (1) {
                ...
                if ($cg_fb_from != 'events') {
                    ...
                    if($cg_fb_source == 'profile' && count($items) == 0){
                        ...
                        $curl_url = 'https://www.facebook.com/platform/plugin/tab/renderer/?key=timeline&config_json=%7B%22app_id%22%3A%222012669838949787%22%2C%22href%22%3A%22https%3A%2F%2Fwww.facebook.com%2F'.$cg_fb_page_id.'%22%2C%22width%22%3A340%2C%22height%22%3A500%2C%22has_cta%22%3Atrue%2C%22has_small_header%22%3Afalse%2C%22has_adapt_container_width%22%3Atrue%2C%22has_cover%22%3Atrue%2C%22has_posts%22%3Afalse%2C%22tabs%22%3A%22timeline%22%2C%22can_personalize%22%3Afalse%2C%22is_xfbml%22%3Atrue%2C%22referer_uri%22%3A%22%22%7D&__a=1&__req=1';
                        //$curl_url = 'https://www.facebook.com/platform/plugin/tab/renderer/?key=timeline&config_json=%7B%22app_id%22%3A%222012669838949787%22%2C%22href%22%3A%22https%3A%2F%2Fwww.facebook.com%2Fcnn%22%2C%22width%22%3A340%2C%22height%22%3A500%2C%22has_cta%22%3Atrue%2C%22has_small_header%22%3Afalse%2C%22has_adapt_container_width%22%3Atrue%2C%22has_cover%22%3Atrue%2C%22has_posts%22%3Afalse%2C%22tabs%22%3A%22timeline%22%2C%22can_personalize%22%3Afalse%2C%22is_xfbml%22%3Atrue%2C%22referer_uri%22%3A%22%22%7D&fb_dtsg_ag=null&__user=0&__a=1&__req=1&__hs=20013.BP%3Aplugin_default_pkg.2.0..0.0&dpr=2&__ccg=EXCELLENT&__rev=1017428696&__s=%3A%3Aciksxi&__hsi=7426725298023643370&__dyn=7wKxa13wt8K2Wmh0Sw8W5U4e1Fx-ewpU3WwvE3vx60Vo1upE4W0OE2Wwce1xwEw7Bx61vw5zw78w5Uw64w8W1uwc-0pa0h-0Lo6-0uS0ue0QU3yw&__csr=&locale=en_US&__sp=1';
                        ...
                        if(stristr($exec,'{"redirect":')){
                            echo '<br>Redirect found, trying to load the redirect URL';
                            preg_match('!{"redirect":"(.*?)"!', $exec, $matches);
                            $redirect_url = $matches[1];

                            //fix json part 
                            $redirect_url = wp_automatic_fix_json_part($redirect_url);

                            echo '<br>Follow the Redirect URL:' . $redirect_url;
                        ...
                        if(isset($matches[1]) && count($matches[1]) > 0){
                           ...
                           //if items is an array and contains more than one, reverse items to get the latest first
                ...
                foreach ($items as $item) {
                    ...
                    if ($cg_fb_from == 'events') {
                        ...
                        $found_story_text = isset($items[0]) ? $items[0] : '';
                    ...
                    //if contains "/reel/", set type to video
                    $is_reel = false; // ini
                    if (stristr($redirec_url, '/reel/')) {
                        echo '<br>Adjusting type to reel';
                        $type = 'video';
                        $is_reel = true;

                        //extract reel Id from https://web.facebook.com/reel/1695961787645819/
                        preg_match('!/reel/(\d+)/!', $redirec_url, $reel_matches);

                        if (isset($reel_matches[1]) && wp_automatic_trim($reel_matches[1]) != '') {
                            $reel_id = $reel_matches[1];
                        }
                    ...
                    //remove timeline stories using regex starting from "__isFeedUnit":"Story","post_id.*
                    $exec2 = preg_replace('!__isFeedUnit":"Story","post_id.*?script>!s', '', $exec2);
                    ...
                    if (wp_automatic_trim($created_time) == '') {
                        ...
                        if (in_array('OPT_YT_DATE', $camp_opt) && wp_automatic_trim($created_time) != '') {
                            if ($this->is_link_old($camp->camp_id, ($created_time))) {
                                echo '<--old post excluding all coming older posts';
                                $foundOldPost = true;
                                break;
                            }
                        }
                    ...
                    if (  (!stristr($item, '</p></span>') && wp_automatic_trim($txtContent) != '' && !$is_colored_post)  ) {
                        ...
                        } elseif (  (stristr($exec2, 'story":{"message":{"text') || preg_match('!"story":.*?"message".*?"text"!', $exec2))) {
                    ...
                    if (stristr($exec2, 'attachments_info')) {
                      ...
                      //"full_image":{"height":2000,"uri":"https:\/\/scontent  
                    }elseif( stristr($exec2,'"full_image":{')){

                        //match
                        preg_match_all('!"full_image":.*?"uri":"(.*?)"!', $exec2, $photo_matches);

                        if(isset($photo_matches[1]) && is_array($photo_matches[1])){
                            $full_imgs_srcs = $photo_matches[1];
                            
                            $candidate_img = $full_imgs_srcs[0];

                            //fix json part
                            if(! empty($candidate_img) && stristr($candidate_img, 'https')){
                                
                                $candidate_img = wp_automatic_fix_json_part($candidate_img);
                                
                                $full_imgs_srcs = array($candidate_img);
                            }

                            
                            $all_imgs[0] = $full_imgs_srcs[0];

                            echo '<br>Image found using full_image ';
                    ...
                    if ($type == 'link') {
                        ...
                        if (stristr($item, 'youtube.com%2Fwatch')) {
                            ...
                            //video title ,"message":{"text":"Luigi Mangione,
                            preg_match('!,"message":{"text":"(.*?)"!s', $exec2, $vid_title_match);
                           ...
                           //"Video","preferred_thumbnail
                            preg_match_all('!ideo","preferred_thumbnail":{"image":{"uri":"(.*?)"!', $exec2, $imgMatch);
                            ...
                            //message
                            $message = isset($vid_title_match[1]) ? wp_automatic_fix_json_part($vid_title_match[1]) : '';
 
                            //echo length
                            echo '<br>Message length:' . strlen($message);

                            //set the txtContent to the message for title generation
                            $txtContent = $message;

                            //add message to the content 
                            $content .= wp_automatic_trim($message) != '' ?  '<p>'. $message . '</p>' : '';
                            ...
                            //if is reel set the video Id to the reel ID
                            if($is_reel && isset($reel_id) ){
                                $vid_id = $reel_id;
                                $id_match = array($reel_id);
                            }
                            ...
                            //if reel, use reel embed code 
                            //<iframe src="https://www.facebook.com/plugins/video.php?height=476&href=https%3A%2F%2Fweb.facebook.com%2Freel%2F886451333459744%2F&show_text=false&width=267&t=0" width="267" height="476" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share" allowFullScreen="true"></iframe>
                            if($is_reel == true){
                                $ret['vid_embed'] = '<iframe src="https://www.facebook.com/plugins/video.php?height=476&href=https%3A%2F%2Fweb.facebook.com%2Freel%2F'.$vid_id.'%2F&show_text=false&width=267&t=0" width="267" height="476" style="border:none;overflow:hidden" scrolling="no" frameborder="0" allowfullscreen="true" allow="autoplay; clipboard-write; encrypted-media; picture-in-picture; web-share" allowFullScreen="true"></iframe>';
                            }
                            ...
                            if ((defined('PARENT_THEME') && (PARENT_THEME == 'truemag' || PARENT_THEME == 'newstube')) || class_exists('Cactus_video')) {
                                ...
                                if (!in_array('OPT_FB_VID_SKIP', $camp_opt)) {
                                    ...
                                    foreach ($id_match as $vid_id_single) {
                                        ...
                                        if($is_reel){
                                            $content .= '[fb_vid ' . $vid_mute . $vidAuto . ' id="' . $vid_id_single . '" reel="1"]';
                                        }else{
                    ...
                    if ($isEvent == true) {
                        ...
                        if (isset($event_desc_matches[1]) && wp_automatic_trim($event_desc_matches[1]) != '') {
                            ...
                            //add the image 
                            if(!empty($imgsrc)){
                                $ret['matched_content'] = '<img src="' . $imgsrc . '" />' . $ret['matched_content'];
                            }
