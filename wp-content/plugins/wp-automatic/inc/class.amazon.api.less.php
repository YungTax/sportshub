...
class wp_automatic_amazon_api_less
{
    ...
    public function getItemByAsin($asin_code, $slug = '')
    {
        ...
        } elseif ( preg_match('{<span class="a-size-large a-color-price">([^<]*?)</span>}s', $exec, $possible_price_matches) ) {
        ...
        if ($elements->length > 0) {
            ...
            foreach ($elements as $element) {
                ...
                @$doc_review->loadHTML( '<html><head>' . $charSetMeta . '</head><body>' . $review . '</body></html>');
                ...
                //build review text
                $review = '<br><br><b>Reviewer:</b> ' . $reviewer_name . '<br><b>Rating:</b> ' . $review_rating . '<br><b>Title:</b> ' . $review_title . '<br><b>Review:</b> ' . $review_text;
